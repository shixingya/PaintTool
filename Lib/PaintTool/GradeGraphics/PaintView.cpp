...
void PaintView::onAssemblageAction(bool)
{
    ...
    QString str =_localDir;
    QString fileName =QString::number(getTimeStamp()).append(".png");
    str.append("/").append(fileName);
    str.replace("/","\\");
    QPainter painter(&image);
    painter.setRenderHint(QPainter::Antialiasing);
    scene->render(&painter);
    bool saveSuccess =  image.save(str);
    ...
    if (!saveSuccess)
    {
        QMessageBox::warning(NULL,"","保存图元到本地失败!");
    }

    SafeDelete(scene);
    emit uploadAssemblageItems(str);
...
void PaintView::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if(selectItemsCount >0)
    {
        ...
        QAction *assemblageAction = menu.addAction("上传到云批改");
...
{
    //    scale(rect().width()/scene()->sceneRect().width(), rect().height()/scene()->sceneRect().height());
    //    scene()->setSceneRect(rect());
...
bool PaintView::checkIsTextIsEditState()
{
    GraphicsTextItem *item = qgraphicsitem_cast<GraphicsTextItem*>(this->scene()->focusItem());
    if(item)
    {
        if(item-> isDuringEdit)
            return true;
    }
    return false;
}
void PaintView::keyPressEvent(QKeyEvent *event)
{
    QList<QGraphicsItem *>  selectItems=this->scene()->selectedItems();
    int selectItemsCount =selectItems.count();
    if(checkIsTextIsEditState()) //测试文字输入框是否为编辑状态
    {
