...
static QGraphicsRectItem * m_selectionRect;
static QRectF selectRectf;
static SelectTool selectTool;
static RubberTool rubberTool;
...
RubberTool::RubberTool():DrawTool(rubber)
{

}

void RubberTool::mousePressEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    DrawTool::mousePressEvent(event,scene);
    scene->views().first()->setMouseTracking(true);
...
void RubberTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    DrawTool::mouseMoveEvent(event,scene);
    scene->mouseEvent(event);
    if (isLeftPushed ==false)
        return;
    QGraphicsItem *itemToRemove = NULL;
    foreach (QGraphicsItem *item, scene->items(event->scenePos())) {
        if (item->type() != 7) {
...
void RubberTool::mouseReleaseEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    isLeftPushed =false;
}
...
void SelectTool::mousePressEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    ...
    if ( item != 0 &&
         item->type() ==GraphicsItem::UserType+1){
...
void SelectTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    ...
    if ( items.count() == 1 )
    {
        item = qgraphicsitem_cast<GraphicsItem*>(items.first());
        if ( item != 0 ){
            if(item->type() !=GraphicsItem::UserType+1)
            {
                scene->mouseEvent(event);
                scene->update();
                return;
...
void ImageTool::mousePressEvent(QGraphicsSceneMouseEvent * event , MainPaintScene * scene )
{
    ...
    item->setRotation(0-scene->getLastRotation());
    scene->addItem(item);
...
void RectTool::mousePressEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    ...
    if(isSquare)
        item->setIsSquare();
    qreal val =scene->getScaleValue();
    item->setBorderWidth(2/val);
...
void PenPathTool::mousePressEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    ...
    case penPath:
        ...
        item = new GraphicsPathItem(*_path,nullptr);
...
void PenPathTool::mouseMoveEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    if(isDrawing)
    {
        QPixmap pixmap(":/Win32/Resources/Win32/pencilTool.png");
        //定义大小
        QSize picSize(8, 8);
        //缩放图片，按比例
        if(!pixmap.isNull())
...
void drawSelectionRect(QPoint p1,QPoint p2)
{
    int Rwidth,Rheight;
    ...
    QRect m_rect(p1,p2);
    if(m_selectionRect)
        m_selectionRect->setRect(m_rect);
    selectRectf.setRect(p1.x(),p1.y(),Rwidth,Rheight);
...
{
    isDrawing=false;
    drawSelectionRect(item->line().p1().toPoint(),item->line().p2().toPoint());
    scene->removeItem(m_selectionRect);
    if(m_selectionRect)
        scene->setSelectionArea(m_selectionRect->shape());
...
void ArrowItem::mousePressEvent(QGraphicsSceneMouseEvent *event, MainPaintScene *scene)
{
    ...
    case arrowLine:
        ...
        _path = new QPainterPath;
        _path->moveTo(event->scenePos());
        item = new GraphicsArrowItem(*_path);
        break;
    }
    if ( item == 0) return;
    isDrawing =true;
